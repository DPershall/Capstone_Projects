...
Now that is accomplished, I can plot the average rating based on release year
...
Effect Model. Now that the brief exploration and proof of improvement for a 
regularized model is behind us we can move forward by expanding our model with
...
Time to show the results of the regularized movie and user effect model.
```{r}
RMSE_results <- bind_rows(RMSE_results,
...
# I USED CLOUD COMPUTING ON AWS TO RUN THIS CODE AS MY MACHINE CANNOT HANDLE 
# THE MEMORY LOAD
...
e <- evaluationScheme(movielenseM, method="split", 
                      train=0.8, k=1, given=-10, goodRating = 3.5)
e
...
opts <- r$tune(train_data, opts = list(dim = c(10, 20, 30),
                                       ...
                                       costq_l1 = 0,                    
                                       lrate    = c(0.01, 0.1),         
                                       nthread  = 8, # WARNING (Select according to your cpu!)              
                                       niter    = 10,                   
                                       verbose  = FALSE)) 
# Train the algorithm
...
# 90/10.
e <- evaluationScheme(movielenseM, method="split", 
                      train=0.9, k=1, given=-10, goodRating = 3.5)
e
