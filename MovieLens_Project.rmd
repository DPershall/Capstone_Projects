...
I will walk through the Recommenderlab method for matrix factorization first. 
The recommenderlab packages are very popular because they can be used with 
several other algorithms like Funk SVD or Item Based Collaborative Filtering.
The chunk below will use the LIBMF library (Library for Matrix Factorization). 
I do not recommend running this chunk on your personal machine because it will 
crash R if you do not have at least 32GB of Ram. I ran this project on AWS to 
save my hardware. The recommenderlab model is kept in the project in order to 
make broader points about large data sets and the challenges they present.
...
gc()
######################### END OF CHUNK UNDER WARNING  ##########################
```

This is a better result! However, the time and machine requirements are not 
feasible for my peers to review on their own machines. This problem is a common
one. Large data sets tend to eat up too much time and machine power for 
computation. However, the solution is quite simple. That is to compute the 
calculations in parallel. Indeed, this problem is so common that it has a 
name and a wikipedia page devoted to it. It is called "Embarrassingly Parallel". 
You can read about it by following the link below.
...
working through the different models and examining the results. I am pleased to 
have three final validations that surpass the class goal of an RMSE below 
.86490, and I am happy to have met my personal goal of an RMSE below 0.80. 
